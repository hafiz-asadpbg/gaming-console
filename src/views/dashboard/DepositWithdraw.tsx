...
const DepositWithdraw = () => {
  return (
    <Card sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: ['column', 'column', 'row'], backgroundColor:'#333' }}>
      <Box sx={{ width: '100%' }}>
        <CardHeader
          title='Deposit'
          color='#fff'
          sx={{ pt: 5.5, alignItems: 'center', '& .MuiCardHeader-action': { mt: 0.6 }, color:'#fff!important' }}
        ...
        <CardContent sx={{ pb: theme => `${theme.spacing(5.5)} !important` }}>
          {depositData.map((item: DataType, index: number) => {
            return (
              <Box
                key={item.title}
                sx={{ display: 'flex', alignItems: 'center',color:'#fff', mb: index !== depositData.length - 1 ? 6 : 0 }}
              >
                ...
                >
                  <Box sx={{ marginRight: 2, display: 'flex', flexDirection: 'column' }}>
                    <Typography sx={{ fontWeight: 600, fontSize: '0.875rem', color: '#fff' }}>{item.title}</Typography>
                    <Typography variant='caption' sx={{color: '#fff'}}>{item.subtitle}</Typography>
                  </Box>
      ...
      <Box sx={{ width: '100%' }}>
        <CardHeader
          title='Withdraw'
          sx={{ pt: 5.5, alignItems: 'center', '& .MuiCardHeader-action': { mt: 0.6 }, color: '#fff' }}
          action={<Typography variant='caption'>View All</Typography>}
          titleTypographyProps={{
            variant: 'h6',
            sx: { lineHeight: '1.6 !important', letterSpacing: '0.15px !important' }
